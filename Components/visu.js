...
var Visu = {
    ...
    createItems: (parent, listContent, sortingTag = false, lvl = 0, ParentZone = true) => {
        ...
        if (sortingState && lvl == 0) {
            if (sortingName) {
                listContent.innerHTML = "<a onclick='Nav.tag=null;changeItem(Nav.uid);Nav.updateUrl(true);'>" + miDb.dico['exitSortingMode'] + "</a>";
            } else {
                listContent.innerHTML = "<a onclick='Nav.tag=null;changeItem(Nav.uid);Nav.updateUrl();'>" + miDb.dico['exitSortingMode'] + "</a>";
            }
            if (sortingTag) {

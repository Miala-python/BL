    ...
    loadJSON('./data/changelog.mi', (data) => {
        ...
        data.split('\n').forEach(line => {
            if (line.startsWith('>')) {
                ...
                const newItem = document.createElement(`h${level}`);
                ...
                listStack[level].appendChild(newItem);
            }
        });

        document.getElementById('changelog').appendChild(changeLogContainer);
    });
}